...
- 先检查现有项目是否已经启用图标库或图标组件；存在时必须复用，避免平行图标体系。
- 最终页面的导航、通知、用户、历史消息、下拉、附件、发送等普通图标不得使用 Unicode 字符、emoji、CSS 手绘近似图或临时 SVG path。
- 当前是无依赖原生 HTML 时，新增 npm 包、CDN 或图标字体会改变运行依赖，必须先让用户选择。用户批准 CDN 时优先使用固定版本的 Lucide UMD，并在库加载后统一调用 `lucide.createIcons()`；禁止引用 `latest`。
- 图标按钮必须有 `aria-label` 或可见文本，陌生图标需有 `title`/tooltip；用稳定宽高和 `stroke-width` 校准，避免加载前后布局跳动。
...
开始前先创建隔离预览目录，只复制 HTML、CSS、JS 和公开静态资源。不得直接把包含 `.env.local`、API Key、凭据或无关私有文件的仓库根目录作为静态服务根目录。预览服务绑定 `127.0.0.1`，并验证敏感文件不在预览目录中。
