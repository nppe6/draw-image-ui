...
CODEX_API_STYLES = ("responses", "images")
...
def resolve_project_env(*keys: str) -> str:
    for key in keys:
...
def resolve_api_key() -> str:
    key = resolve_project_env("ZENMUX_API_KEY")
    if key:
        return key
...
def build_output_path(*, output_arg: str, image_type: str, topic: str, explicit_name: str, ext: str) -> Path:
    ...
    try:
        ...
        day_dir = Path.cwd().resolve() / "output" / "draw" / now.strftime("%Y-%m-%d")
        day_dir.mkdir(parents=True, exist_ok=True)
...
def resolve_codex_model(override: str = "") -> str:
    return override.strip() or resolve_project_env("DRAW_CODEX_MODEL") or DEFAULT_CODEX_MODEL
...
def resolve_image_field() -> str:
    return resolve_project_env("OPENAI_IMAGE_FIELD") or "image[]"
    ...
    if isinstance(value, dict):
        ...
        for key in ("result", "b64_json", "image_base64"):
            ...
            if isinstance(item, str) and (
                key == "b64_json" or value_type == "image_generation_call" or looks_like_base64_image(item)
            ):
...
def encode_multipart(fields: list[tuple[str, str]], files: list[tuple[str, Path]]) -> tuple[bytes, str]:
    boundary = f"----draw-image-ui-{os.urandom(12).hex()}"
    chunks: list[bytes] = []
...
def build_images_api_request(
    *, base_url: str, api_key: str, prompt: str, refs: list[Path], model: str, size: str, quality: str
) -> urllib.request.Request:
    headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json"}
    if refs:
        endpoint = join_endpoint(base_url, "images/edits")
        fields = [
            ("model", model),
            ("prompt", prompt),
            ("size", size),
        ...
        data, boundary = encode_multipart(
            fields,
            [(resolve_image_field(), ref) for ref in refs],
        )
...
def request_codex_image(
    *,
    ...
    if resolved_style == "images":
        request = build_images_api_request(
            base_url=base_url,
            api_key=api_key,
            prompt=prompt,
            refs=refs,
        ...
        payload = {
            ...
            "tools": [{"type": "image_generation", "size": resolved_size}],
    ...
    try:
        ...
        raise RuntimeError(
            f"HTTP {exc.code} from OpenAI-compatible image API ({resolved_style}):\n{details}"
        ) from exc
    ...
    if not image_b64:
        raise RuntimeError(f"No image result found in OpenAI-compatible {resolved_style} API output.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--quality", choices=IMAGE_QUALITIES, default="high", help="Image quality for Images API."
    )
...
def main() -> int:
    ...
    api_style = resolve_codex_api_style(args.api_style) if args.provider == "codex" else ""
    ...
    with tempfile.TemporaryDirectory(prefix="draw-refs-") as tmp:
        ...
        metadata = {
            ...
            "model": model,
            "api_style": api_style,
            "size": args.size or (CODEX_SIZE_PRESETS.get(args.type) if args.provider == "codex" else ""),
            "quality": args.quality,
