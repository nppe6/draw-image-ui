...
### Standard Icon Contract

- Inspect the dependency manifest and existing components before choosing icons. Reuse the installed icon library and local wrapper component when present.
- Do not ship Unicode symbols, emoji, CSS-drawn approximations, or improvised SVG paths as final navigation, toolbar, account, notification, history, or send icons.
- If no icon system exists, adding a package, CDN, iconfont, or copied SVG set changes project dependencies or asset policy. Ask the user which route to use before editing.
- For standalone HTML after CDN approval, prefer a pinned Lucide UMD version and initialize it once after the library loads. Do not use `latest`.
- Keep icon size, stroke width, alignment, accessible label, hover state, and disabled state stable so icons cannot resize controls.
...
Never serve the repository root when it contains `.env*`, API keys, credentials, private fixtures, or unrelated files. Create an isolated preview directory containing an allowlist of the built page and required public assets, then bind the preview server to `127.0.0.1`. If the framework requires its normal dev server, verify that secret files are not reachable as static content before opening the browser.
...
```powershell
python "$env:USERPROFILE\.agents\skills\draw-image-ui\scripts\compare_mockup.py" `
